...
for current in daterange(start, end):
    ...
    url = "http://www.bom.gov.au/web03/ncc/www/awap/solar/solarave/daily/grid/0.05/history/nat/%s%s%s%s%s%s.grid.Z" % \
         (
            str(current.year).zfill(4), str(current.month).zfill(2), str(current.day).zfill(2), 
            str(current.year).zfill(4), str(current.month).zfill(2), str(current.day).zfill(2)
         )

    # Download and save a local copy of the grid file (solar irradiance)
